<x-admin-layout>
    ...
    <div x-data="{ showPermanentDeleteModal: false, customerId: null }">
        <div x-show="showPermanentDeleteModal" 
            ...
            <div class="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity" @click="showPermanentDeleteModal = false"></div>
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6"
                    ...
                    <div class="flex justify-end space-x-3">
                        <button @click="showPermanentDeleteModal = false" 
                                class="px-4 py-2 bg-white text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50">
                            Avbryt
                        </button>
                        <form :action="`{{ url('admin/customers') }}/${customerId}/force`" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
        ...
        <button @click="showPermanentDeleteModal = true; customerId = $event.detail.customerId" 
                id="permanentDeleteModalTrigger" 
                class="hidden"></button>
    </div>

    <script>
        function confirmPermanentDelete(customerId) {
            const event = new CustomEvent('click', { detail: { customerId: customerId } });
            document.getElementById('permanentDeleteModalTrigger').dispatchEvent(event);
        }
    </script>
</x-admin-layout>
