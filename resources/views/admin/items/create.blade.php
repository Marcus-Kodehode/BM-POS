<x-admin-layout>
    <x-slot name="title">Opprett vare</x-slot>

    <div class="max-w-2xl">
        <!-- Header -->
        <div class="mb-6">
            <h2 class="text-2xl font-semibold text-gray-900">Opprett ny vare</h2>
            <p class="mt-1 text-sm text-gray-600">
                Legg til en ny vare i lageret. Status settes automatisk til "Tilgjengelig".
            </p>
        </div>
            ...
            <form method="POST" action="{{ route('admin.items.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-150 @error('description') border-danger-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm font-medium text-primary-800">
                                Varen vil automatisk få status "Tilgjengelig" ved opprettelse.
                            </p>
                        </div>
                    </div>
                </div>
